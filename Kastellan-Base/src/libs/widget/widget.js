...
// the object
 {
   // The identifier is read only. Using a getter protects it
	...
	// hide _classID, it is not needed outside
	var _classId = widget.classIdForInstance(_instanceId);
...
// --- the widgetFrame class gives a widget access to the frame surrounding it
 {
    ...
    function() {
        return widget.preferenceForKey(_instanceId + "/kastellan/iconurl");
    });
	// used to be widget.setIconName (kind of, see widgetBundle object)
    // widget.frame.iconURL = someVar;
    this.__defineSetter__("iconURL",
    function(iconURL) {
        widget.setPreferenceForKey(iconURL, _instanceId + "/kastellan/iconurl");
    });
    ...
    this.title = widget.preferenceForKey(_instanceId + "/kastellan/title");
    this.setTitle = function(title) {
    	widget.setPreferenceForKey(title , _instanceId + "/kastellan/title");
    }
}
...
function widgetViewer(identifier)
